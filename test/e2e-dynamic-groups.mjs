...
async function run() {
  ...
  const { response: listRes1 } = await sendAndReceiveById(proc, makeRequest('tools/list', {}, 2), 2);
  ...
  const { response: catRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: listRes2 } = await sendAndReceiveById(proc, makeRequest('tools/list', {}, 4), 4);
  const toolNames2 = listRes2.result.tools.map((t) => t.name);
  assert(
    toolNames2.includes(sanitizeToolName('create_animation')),
    'create_animation now exposed after activation',
  );
  ...
  assert(
    toolNames2.includes(sanitizeToolName('connect_animation_states')),
    'connect_animation_states now exposed',
  );
  assert(
    toolNames2.length === 33 + 5,
    `Tool count after animation activation = ${toolNames2.length} (expected 38)`,
  );
  ...
  const { response: catRes2 } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: activateRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: listRes3 } = await sendAndReceiveById(proc, makeRequest('tools/list', {}, 7), 7);
  ...
  assert(
    toolNames3.includes(sanitizeToolName('dap_set_breakpoint')),
    'dap_set_breakpoint exposed (dap group)',
  );
  assert(
    toolNames3.includes(sanitizeToolName('dap_get_stack_trace')),
    'dap_get_stack_trace exposed (dap group)',
  );
  assert(
    toolNames3.length === 33 + 5 + 4 + 6,
    `Tool count with 3 groups = ${toolNames3.length} (expected 48)`,
  );

  // ── Phase 5: manage_tool_groups status & list ──────────────
  console.log('\n[Phase 5] manage_tool_groups status & list');

  const { response: statusRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  assert(
    statusData.dynamicGroups.activeCount === 3,
    `Active group count = ${statusData.dynamicGroups.activeCount} (expected 3)`,
  );
  ...
  const { response: listGroupsRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: deactRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: listRes4 } = await sendAndReceiveById(proc, makeRequest('tools/list', {}, 11), 11);
  const toolNames4 = listRes4.result.tools.map((t) => t.name);
  assert(
    !toolNames4.includes(sanitizeToolName('create_audio_bus')),
    'create_audio_bus hidden after deactivation',
  );
  ...
  assert(
    toolNames4.length === 33 + 5 + 6,
    `Tool count after audio deactivation = ${toolNames4.length} (expected 44)`,
  );

  // ── Phase 7: Reset ──────────────────────────────────────────
  console.log('\n[Phase 7] Reset all groups');

  const { response: resetRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  assert(
    resetData.deactivated.length === 2,
    `Deactivated ${resetData.deactivated.length} groups (animation + dap)`,
  );

  await drainNotifications(proc, 300);

  const { response: listRes5 } = await sendAndReceiveById(proc, makeRequest('tools/list', {}, 13), 13);
  assert(
    listRes5.result.tools.length === 33,
    `After reset: ${listRes5.result.tools.length} tools (expected 33)`,
  );
  ...
  const { response: actAutoload } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: autoloadRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: actDx } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: healthRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: badGroupRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  assert(
    badData.error?.includes('Unknown group'),
    `Invalid group returns error: ${badData.error?.substring(0, 50)}...`,
  );

  // Duplicate activation should be idempotent
  const { response: dupRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: bpRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: themeRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
  ...
  const { response: navRes } = await sendAndReceiveById(
    proc,
    makeRequest(
      'tools/call',
      {
