...
func delete_node(params):
	...
	var result = {
		"scene_path": params.scene_path,
		"deleted_node": {"name": deleted_name, "type": deleted_type, "parent": parent_name},
		"scene_saved": save_scene_after
	}
...
func duplicate_node(params):
	...
	var result = {
		...
		"duplicated_node": {"name": new_name, "type": duplicate.get_class(), "parent": target_parent.name},
		"scene_saved": save_scene_after
	}
...
func reparent_node(params):
	...
	var result = {
		"scene_path": params.scene_path,
		"reparented_node":
		{"name": node_name, "type": node_type, "old_parent": old_parent_name, "new_parent": new_parent.name},
		"scene_saved": save_scene_after
	}
...
func connect_signal(params):
	...
	var connection_line = (
		...
		+ signal_name
		+ '" from="'
		+ source_path_in_scene
		+ '" to="'
		+ target_path_in_scene
		+ '" method="'
		+ method_name
		+ '"'
	)
...
func disconnect_signal(params):
	...
	for line in lines:
		if (
			line.strip_edges().begins_with("[connection")
			and line.contains('signal="' + signal_name + '"')
			and line.contains('from="' + source_path_in_scene + '"')
			and line.contains('to="' + target_path_in_scene + '"')
			and line.contains('method="' + method_name + '"')
		):
			found = true
			if debug_mode:
	...
	for line in lines:
		...
		if trimmed.begins_with("[connection"):
			...
			if connection != null:
				...
				if (
					filter_scene_path.is_empty()
					or connection["source"] == filter_scene_path
					or connection["target"] == filter_scene_path
				):
					connections.append(connection)

	var result = {"scene_path": params.scene_path, "connections": connections, "count": connections.size()}
...
func parse_connection_line(line: String) -> Variant:
	...
	var result = {"source": "", "signal": "", "target": "", "method": "", "flags": 0}
...
func create_resource(params):
	...
	if not resource_dir.is_empty() and resource_dir != "res://":
		...
		if dir:
			...
			if (
				not relative_dir.is_empty()
				and not DirAccess.dir_exists_absolute(ProjectSettings.globalize_path(resource_dir))
			):
...
func create_material(params):
	...
	if not material_dir.is_empty() and material_dir != "res://":
		...
		if dir:
			...
			if (
				not relative_dir.is_empty()
				and not DirAccess.dir_exists_absolute(ProjectSettings.globalize_path(material_dir))
			):
				var err = dir.make_dir_recursive(relative_dir)
				if err != OK:
...
func create_shader(params):
	...
	if not shader_dir.is_empty() and shader_dir != "res://":
		...
		if dir:
			...
			if (
				not relative_dir.is_empty()
				and not DirAccess.dir_exists_absolute(ProjectSettings.globalize_path(shader_dir))
			):
				var err = dir.make_dir_recursive(relative_dir)
				if err != OK:
	...
	var result = {
		...
		"template_used":
		template if not template.is_empty() else ("custom" if not code.is_empty() else "basic"),
...
func create_animation(params):
	...
	if not player_node is AnimationPlayer:
		log_error(
			"Node is not an AnimationPlayer: " + player_node_path + " (type: " + player_node.get_class() + ")"
		)
		scene_root.queue_free()
		quit(1)
	...
	if debug_mode:
		log_debug(
			(
				"Animation created with length: "
				+ str(animation.length)
				+ ", loop_mode: "
				+ str(animation.loop_mode)
			)
		)
...
func add_animation_track(params):
	...
	if track_type_str == "property":
		...
		for keyframe in keyframes:
			...
			var method_call = {"method": method_name, "args": args}
...
func create_animation_tree(params: Dictionary):
	...
	match root_type:
		...
		"BlendSpace1D":
			tree_root = AnimationNodeBlendSpace1D.new()
		"BlendSpace2D":
			tree_root = AnimationNodeBlendSpace2D.new()
		_:
			tree_root = AnimationNodeStateMachine.new()
...
func set_theme_color(params: Dictionary):
	...
	theme.set_color(
		color_name,
		control_type,
		Color(float(color.r), float(color.g), float(color.b), float(color.get("a", 1.0)))
	)
